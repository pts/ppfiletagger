...
* For ppfiletagger_shell_functions.sh (_mmfs), on FreeBSD, NetBSD and
  Solaris only, the https://metacpan.org/pod/File::ExtAttr Perl module
  installed (i.e. the `perl -mFile::ExtAttr -edie' command should print
  `Died at -e line 1.'). On Linux and macOS, this Perl module is not needed,
  it's functionality is built in.

Fast search (building the full index (rmtimescan) and doing fast searches
(_mmfs query, rmtimequery)) needs:
...
`_mmfs find' and `_mmfs query' (rmtimequery) implement a sophisticated
search query language with operators like `-', `|` and `ext:`, see details
and syntax here: https://github.com/pts/locfileorg/blob/master/doc/search.md
...
starting with `user.', but only user.mmfs.tags is searchable by `_mmfs query'.
...
`_mmfs query' should take less than 1 second in most use cases, and a few
seconds for complicated, long queries (e.g. `* -ext:rare000').

The speed of `rmtimescan --slow' depends on the medium (HDD or SSD),
